    ...
    public class ThietBiYTeRepository:IThietBiYteRepository
    {
        ...
        public async Task<byte[]> DownloadPdfFile(int entity)
        {
            try
            {
                ...
                {
                    throw new Exception("Danh sách thiết bị rỗng!");
                }
                ...
                using (var memoryStream = new MemoryStream())
                {
                    using (var document = new Document())
                    {
                        ...
                        foreach (var entity1 in danhSachXuatThuoc)
                        {
                            ...
                            table.AddCell(new PdfPCell(new Phrase(entity1.tenThietBiYTe, columnHeaderFontnd)));
        ...
        public async Task<byte[]> CreatePdfInvoices(List<int> maHDList)
        {
            using (var memoryStream = new MemoryStream())
            {
                ...
                document.Add(new Paragraph($"Mã Lịch Hẹn: {maHDListStr}", contentFont));
                ...
                foreach (var sanPham in sanPhamList)
                {
                    if (sanPham is thuocMuaModel thuoc)
                    {
                        ...
                        var loThuocList = await _context.loThuocEntities
                                              .Where(t => t.MaThuoc == thuoc.maSanPham && t.soLuong > 0)
        ...
        public void PrintPdf(string pdfPath)
        {
            ...
            string fontAbsolutePath = System.IO.Path.Combine(fontDirectory, "gswin64c.exe");

            // Kiểm tra sự tồn tại của file font
            if (!System.IO.File.Exists(fontAbsolutePath))
            {
                throw new FileNotFoundException("File font không tồn tại tại đường dẫn: " + fontAbsolutePath);
            }
