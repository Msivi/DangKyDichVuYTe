...
namespace Backend_DV_YTe.Repository
{
    public class ThanhToanDVRepository : IThanhToanDVRepository
    {
        ...
        public async Task<string> PrintInvoiceAsync(Invoice invoice)
        {
            ...
            // Đường dẫn trên host
            var pdfPath = System.IO.Path.Combine(@"h:\root\home\levi123-001\www\site1\HoaDonDV", $"hoaDonDichVu_{timestamp}.pdf");
        ...
        public byte[] CreatePdfInvoice(Invoice invoice)
        {
            using (var memoryStream = new MemoryStream())
            {
                ...
                string fontDirectory = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Fonts");
                string fontAbsolutePath = System.IO.Path.Combine(fontDirectory, "arial.ttf");
